...
class AnalysisConfig:
    ...
    betas: list[float] = field(default_factory=lambda: [0.5, 1.0, 2.0, 3.0])
...
def score_models(grid: pd.DataFrame, config: AnalysisConfig) -> dict:
    ...
    for col in model_cols:
        ...
        threshold_sweeps[display_name] = [
            {
                ...
                "f2": float(rl.fbeta_at_threshold(grid[col], grid["rain_truth"], int(idx), beta=2.0)) if not np.isnan(rl.fbeta_at_threshold(grid[col], grid["rain_truth"], int(idx), beta=2.0)) else None,
            }
        ...
        for beta in config.betas:
            # Apply min_precision for high beta values to avoid useless always-on thresholds
            min_prec = 0.6 if beta >= 2.0 else 0.0
            rec = rl.recommend_threshold(grid[col], grid["rain_truth"], beta=beta, min_precision=min_prec)
...
def generate_summary(report: dict) -> str:
    ...
    header = f"  {'Model':<22} {'Prec':>6} {'Recall':>6} {'F1':>6} {'F2':>6} {'TP':>5} {'FP':>5} {'FN':>5}"
    lines.append(header)
    lines.append("  " + "-" * len(header) + "--------")
    for name, s in scores.items():
        # Calculate F2 for comparison table
        f2_val = None
        for sweep in report["scoring"]["threshold_sweeps"].get(name, []):
            if sweep["threshold"] == meta["config"]["decision_threshold"]:
                f2_val = sweep.get("f2")
                break
        
        if s["precision"] is not None:
            f2_str = f"{f2_val:>6.3f}" if f2_val is not None else "  N/A"
            lines.append(
                f"  {name:<22} {s['precision']:>6.3f} {s['recall']:>6.3f} {s['f1']:>6.3f} {f2_str} "
                f"{s['tp']:>5} {s['fp']:>5} {s['fn']:>5}"
            )
        else:
            lines.append(f"  {name:<22}   N/A   N/A   N/A   N/A {s['tp']:>5} {s['fp']:>5} {s['fn']:>5}")
    lines.append("")
