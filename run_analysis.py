...
def run_models(grid: pd.DataFrame, config: AnalysisConfig) -> tuple:
    ...
    for name, func in MODELS.items():
        col = f"model_{name}"
        # Pass pressure if available (needed by pressure_aware model)
        if name == "pressure_aware" and "pressure" in grid.columns:
            grid[col] = func(grid["spread"], grid["spread_deriv"], params,
