...
def test_forecast_mode_accepts_today_end_date():
    """Test that --use-forecast with --end=today succeeds."""
    ...
    assert result.returncode == 0, f"Should succeed with --end=today, got stderr: {result.stderr}"
...
def test_forecast_mode_default_end_is_today():
    ...
    assert result.returncode == 0, f"Default --end should be today, got stderr: {result.stderr}"
    assert "requires --end to be today" not in result.stderr, \
