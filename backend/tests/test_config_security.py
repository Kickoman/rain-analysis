...
def test_env_example_ships_placeholder_that_validator_rejects():
    """
    Test that .env.example ships with a placeholder that the validator rejects.
    
    This is a deliberate fail-fast design:
    - .env.example contains an obvious placeholder value
    - The validator explicitly rejects that placeholder
    - This forces users to generate their own secure value before the app starts
    - Prevents accidental deployment with the example value
    """
    ...
    # The .env.example should contain the insecure default value
